...
export const getAllProducts = async (name, plu, res) => {
    try {
        ...
        if (name) {
            query.andWhere('LOWER(product.name) LIKE LOWER(:name)', { name: `%${name.toLowerCase()}%` });
        }
        ...
        res.status(500).json({message: 'Ошибка при получении товаров: ' + error.message});
    }
}
...
export const GetProductDetail = async (id) => {
    ...
    return await repo.findOne({where: {id}, transaction: false})
}