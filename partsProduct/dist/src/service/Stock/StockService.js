...
export const IncrementStock = async (id, quantityInOrder, quantityOnShelf, res)=>{
    try {
        ...
        await AppDataSource.getRepository(StockEntity).update({
            id
        }, {
        ...
        return res.status(500).json({
            message: "Ошибка при обновлении остатков",
            error: error.message
        });
    }
};
...
export const getWithFiltersStock = async (req, res)=>{
    try {
        const { plu, shopId, quantityOnShelfMin, quantityOnShelfMax, quantityInOrderMin, quantityInOrderMax } = req.query;
        const stockRepository = AppDataSource.getRepository(StockEntity);
        const queryBuilder = stockRepository.createQueryBuilder("stock");
        // Фильтрация по PLU
        if (plu) {
            const product = await AppDataSource.getRepository(ProductEntity).findOne({
                where: {
                    plu
                },
                transaction: false
        ...
        if (shopId) {
            queryBuilder.andWhere("stock.shopId = :shopId", {
                shopId
            });
        }
        ...
        if (quantityOnShelfMin) {
            queryBuilder.andWhere("stock.quantityOnShelf >= :quantityOnShelfMin", {
                quantityOnShelfMin: Number(quantityOnShelfMin)
            });
        }
        if (quantityOnShelfMax) {
            queryBuilder.andWhere("stock.quantityOnShelf <= :quantityOnShelfMax", {
                quantityOnShelfMax: Number(quantityOnShelfMax)
            });
        }
        ...
        if (quantityInOrderMin) {
            queryBuilder.andWhere("stock.quantityInOrder >= :quantityInOrderMin", {
                quantityInOrderMin: Number(quantityInOrderMin)
            });
        }
        if (quantityInOrderMax) {
            queryBuilder.andWhere("stock.quantityInOrder <= :quantityInOrderMax", {
                quantityInOrderMax: Number(quantityInOrderMax)
            });
        }
