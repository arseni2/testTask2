...
app.get("/actions", async (req, res) => {
    try {
        ...
        if (shopId) {
            queryBuilder.andWhere("action.shopId = :shopId", {shopId});
        }
        if (startDate) {
            queryBuilder.andWhere("action.createdAt >= :startDate", {startDate: new Date(startDate)});
        }
        if (endDate) {
            queryBuilder.andWhere("action.createdAt <= :endDate", {endDate: new Date(endDate)});
        }
        if (action) {
            queryBuilder.andWhere("action.action LIKE :action", {action: `%${action}%`});
        }
        ...
        const idList = [...new Set(actions.map(action => action.productId))]
